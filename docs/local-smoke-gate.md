...
The isolated environment pins PyTorch 2.13.0+cu130, Transformers 4.57.6, PEFT 0.18.0, Accelerate 1.12.0, Tokenizers 0.22.2, Safetensors 0.8.0, and NumPy 2.2.6. `pip check`, Qwen3 class import, `torch.cuda.is_available()`, BF16 support, and a small BF16 CUDA matrix multiplication passed. The model loader requires the complete fixed-revision model in `output/models/qwen3-0.6b-c1899de`, verifies every required file's SHA-256, and loads with `local_files_only=True`. WSL startup was intermittent during setup but recovered for the complete smoke run recorded below.
...
The pinned model download completed with the expected 1,503,300,328-byte size and SHA-256. WSL2 Ubuntu 22.04, Python 3.10.12, PyTorch 2.13.0+cu130, CUDA availability, BF16 support, and `pip check` passed on the RTX 3060 Laptop GPU. The 730-token preflight passed with 2,676 MiB peak allocated, 2,722 MiB peak reserved, and 2,354 MiB free after backward. Two LoRA optimizer steps then ran on original released rows 473 and 758 with losses 1.2940 and 0.5392; peak reserved memory in training was 3,730 MiB. The saved adapter was reloaded in a new process and generated 128 tokens for each of two held-out prompts. All four final-stage manifests were produced against code/config commit `51e7127ab5707be83c40b5d0b538169934b2fe4b`.
