...
The evidence localized the earlier failure to host-side VM creation before Ubuntu boot, but did not prove one underlying cause. A later launch **succeeded without a reboot**, after the temporary configuration had been removed. Ubuntu 22.04, Python 3.10.12, `venv`, the E: repository mount, and RTX 3060 passthrough were rechecked. At that point the GPU reported 5,435 MiB free of 6,144 MiB. WSL's system Python has no `pip`, but an isolated `/root/.venvs/hint-tuning-reproduce` environment was created successfully with its own pip. No WSL reinstall or VHDX change was needed.

The isolated environment pins PyTorch 2.13.0+cu130, Transformers 4.57.6, PEFT 0.18.0, Accelerate 1.12.0, Tokenizers 0.22.2, Safetensors 0.8.0, and NumPy 2.2.6. `pip check` passed. PyTorch reported CUDA available, compute capability 8.6, BF16 support, and completed a small BF16 matrix multiplication; free VRAM during that check was 5,118 MiB. The smoke data slicer also ran in WSL. Fixing its JSON writer to use LF made the Windows and WSL train/eval hashes identical.

Subsequent fresh WSL launches returned the same `0x800705aa` again, including once when Windows reported 5.37 GiB free physical memory and 8.06 GiB free virtual memory. This makes the recovery intermittent and weakens the earlier low-memory-only hypothesis. The venv and CUDA checks are valid evidence from a successful instance, but model download and forward/backward preflight must wait until WSL can start reliably. The next least invasive host action is a normal Windows reboot; do not unregister Ubuntu or alter its VHDX.
